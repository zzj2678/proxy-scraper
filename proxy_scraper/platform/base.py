...
class ProxyScraperBase(abc.ABC):
    ...
    def save_raw_proxies(self, proxies: List[Proxy]):
        raw_proxy_dir = os.path.join(PROXY_DIR, 'raw')

        # Ensure directory exists
        os.makedirs(raw_proxy_dir, exist_ok=True)

        try:
            self.writer.save_raw_proxies(proxies, raw_proxy_dir)
            logger.info(f"Successfully saved {len(proxies)} raw proxies to {raw_proxy_dir}")
        except Exception as e:
            logger.error(f"Failed to save raw proxies: {e}")
    ...
    async def fetch_and_process(self, url):
        ...
        if content:
            ...
            self.save_raw_proxies(valid_proxies)
