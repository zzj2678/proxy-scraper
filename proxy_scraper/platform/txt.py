...
class TxtScraper(ProxyScraperBase):
    ...
    async def scrape(self):
        for url in self.get_urls():
            ...
            self.writer.save_raw_proxies(valid_proxies, os.path.join(PROXY_DIR, "raw"))
            logger.info(f"Saved {len(valid_proxies)} valid proxies from {url} to {os.path.join(PROXY_DIR, 'raw')}")
